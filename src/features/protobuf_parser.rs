...
fn parse_field(data: &[u8]) -> (Field<'_>, &[u8]) {
    ...
    let (fieldvalue, remainder) = match wire_type {
        ...
        WireType::Len => {
            let (len_u64, rem_after_len) = parse_varint(remainder);
            let len = len_u64 as usize;

            if len > rem_after_len.len() {
                panic!("Not enough bytes for Len field");
            }
            let field_data = &rem_after_len[..len]; // From start to len
            let new_remainder = &rem_after_len[len..]; // From len to endA
            (FieldValue::Len(field_data), new_remainder)
        }
    };
    let field = Field {
        field_num,
...
impl<'a> ProtoMessage<'a> for PhoneNumber<'a> {
    fn add_field(&mut self, field: Field<'a>) {
        match field.field_num {
            ...
            _ => {} // Ignore unknown fields (standard protobuf behavior)
        }
    }
}
...
impl<'a> ProtoMessage<'a> for Person<'a> {
    fn add_field(&mut self, field: Field<'a>) {
        match field.field_num {
            ...
            2 => self.id = field.value.as_u64(), // Helper method for Varint
            3 => {
                // It's a nested message!
                let bytes = field.value.as_bytes(); // Get the raw bytes
                let phone_entry = parse_message(bytes); // Recursively parse!
                self.phone.push(phone_entry);
            }
            _ => {} // Ignore unknown
        }
    }
